...
public:
   RBatchGenerator(const std::vector<ROOT::RDF::RNode> &rdfs, const std::size_t chunkSize, const std::size_t blockSize,
                   ...
                   std::string sampleType = "", float sampleStrategy = 1.0, const std::size_t randomState = 42,
        ...
        fUseWholeFile(maxChunks == 0)
   {
      ...
      if (fLoadEager) {
         ...
         if (fSampleType == "") {
            ...
            fTrainingSampler = std::make_unique<RSampler>(fTrainingDatasets, fSampleType, fSampleStrategy, fRandomState, fReplacement,
                                                          fShuffle, fSetSeed);
            fValidationSampler = std::make_unique<RSampler>(fValidationDatasets, fSampleType, fSampleStrategy, fRandomState, fReplacement,
                                                            fShuffle, fSetSeed);
      ...
      fTrainingBatchLoader = std::make_unique<RBatchLoader>(fBatchSize, fCols, fVecSizes, fNumTrainingEntries, fDropRemainder);              fValidationBatchLoader = std::make_unique<RBatchLoader>(fBatchSize, fCols, fVecSizes, fNumValidationEntries, fDropRemainder);
   }
