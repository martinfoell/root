...
public:
  ...
  void ShuffleTensor(RFlat2DMatrix &ShuffledTensor, RFlat2DMatrix &Tensor) {
   ...
   void SliceTensor(RFlat2DMatrix& SlicedTensor, RFlat2DMatrix& Tensor,
                    const std::vector<std::vector<std::size_t>>& slice)
   {
      ...
      std::copy(Tensor.GetData() + rowStart * cols,
                Tensor.GetData() + rowStart * cols + rows * cols,
                SlicedTensor.GetData());
   }
  
   void ConcatinateTensors(RFlat2DMatrix &ConcatTensor, const std::vector<RFlat2DMatrix> &Tensors)
   {
      ...
      for (std::size_t i = 0; i < Tensors.size(); i++) {
         ...
         std::copy(Tensors[i].GetData(),
                   Tensors[i].GetData() + tensorRows * cols,
